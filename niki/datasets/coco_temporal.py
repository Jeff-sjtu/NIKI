...
class coco_temporal(data.Dataset):
    ...
    def evaluate_xyz_17(self, preds, gts, per_jts_err=False):
        ...
        if per_jts_err:
            ...
            error_per_jts = error.mean(axis=0)
            error_align_per_jts = error_align.mean(axis=0)
            for i in range(14):
                print(self.joints_name_14[i], error_per_jts[i], error_align_per_jts[i])
